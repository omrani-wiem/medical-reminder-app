...
export const calculerRetard = (heurePrevu, heurePrise) => {
  if (!heurePrevu || !heurePrise) return null;
  const toMinutes = (t) => {
...
export const exporterCSV = (donnees) => {
  const headers = ['Date', 'Heure', 'Médicament', 'Dosage', 'Statut',
    'Prise Effective', 'Retard', 'Médecin', 'Notes'];
  ...
  const csvContent = [
    headers.join(','),
    ...donnees.map(item => [
      item.date, item.heure, item.medicament, item.dosage,
      item.statut, item.priseEffective || '-',
      item.retard != null ? `${item.retard}min` : '-',
      item.medecin, item.notes || '-'
    ].map(escapeCSV).join(','))
  ].join('\n');
